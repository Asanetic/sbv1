...
// ExpensesProfile — pure shell. It resolves the id, wires up the
...
export default function ExpensesProfile({ id: idProp, onDone, hiddenActions = [], presetValues, schemaOverride }) {
  const searchParams = useSearchParams();
  const schema = schemaOverride || ExpensesSchema;
  const id = idProp ?? searchParams.get(`${ExpensesSchema.entity}_dataNode`);
  const form = useEntityFormController(schema, ExpensesActions, {
    id,
    onDone,
