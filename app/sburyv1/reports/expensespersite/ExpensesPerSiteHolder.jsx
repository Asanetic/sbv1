...
export default function ExpensesPerSiteHolder() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const response = await mosyGetData({
        endpoint: apiRoutes.reports.expensespersite,
        params,
      });
      ...
      if (response?.status === 'success') {
        ...
        setError(response?.message || 'Failed to load expenses per site report');
      }
  ...
  return (
    <div className="epst-wrapper col-md-12 p-0 m-0">
      <div className="epst-header">
        <div>
          <h3 className="epst-title">Expenses per Site</h3>
          <div className="epst-sub">Expenses (expenses.rate) broken down by site.</div>
          <div className="epst-viewing">
            Showing {filters.year} · {monthsLabel}
          </div>
        </div>

        <div className="epst-filters">
          <div className="filter-field">
            <label>Year</label>
            <select className="epst-select" value={draftYear} onChange={(e) => setDraftYear(Number(e.target.value))}>
              {yearOptions.map((y) => (
          ...
          <button type="button" className="epst-apply-btn" disabled={!dirty} onClick={handleApply}>
            Apply
          </button>

          {hasActiveFilters && (
            <button type="button" className="epst-reset-btn" onClick={handleClear}>
              Clear
            </button>
          )}
        </div>
      ...
      {loading ? (
        ...
        <>
          <div className="epst-tiles-row">
            <div className="epst-tile">
              <div className="epst-tile-label">Total Expenses</div>
              <div className="epst-tile-value">{fmtKES(data.total.value)}</div>
              <div className="epst-tile-note">{toNum(data.total.txn_count)} entries · {data.by_site.length} site(s)</div>
            </div>
          </div>

          <div className="row m-0">
            <div className="col-md-6 mb-3">
              <div className="epst-section">
                <h5>Expenses per Site</h5>
                {data.by_site.length === 0 ? (
                  <div className="epst-empty">No expenses for this selection.</div>
                ) : (
                  <ElforgeChart
                    chartType="bar"
                    dataKey="label"
          ...
          <div className="epst-section">
            <div className="epst-section-head">
              <h5>Site Breakdown</h5>
              <div className="epst-export-tray">
                <div
                  className="epst-export-btn"
                  onClick={() => mosyPrintToPdf({ elemId: 'epst_print_card', defaultTitle: 'Expenses per Site Report' })}
                >
                  <i className="fa fa-print mr-1"></i> Print / PDF
                </div>
                <div
                  className="epst-export-btn"
                  onClick={() => exportTableToExcel('epst_data_table', `Expenses per Site ${filters.year}.xlsx`)}
                >
                  <i className="fa fa-file-excel-o mr-1"></i> Export to Excel
                </div>
              </div>
            </div>
            <div className="epst-table-wrap" id="epst_print_card">
              <table className="epst-table" id="epst_data_table">
                <thead>
                  <tr>
                    <th>#</th>
                    <th>Site</th>
                ...
                <tbody>
                  {data.by_site.length === 0 ? (
                    <tr>
                      <td></td>
                      <td colSpan={4}>No expenses for this selection.</td>
                    </tr>
                  ) : (
                    data.by_site.map((row, i) => (
                      <tr key={row.site_id || row.label}>
                        <td>{i + 1}</td>
                        <td>{row.label}</td>
