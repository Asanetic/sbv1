...
// ContractorsProfile — pure shell. It resolves the id, wires up the
...
export default function ContractorsProfile({ id: idProp, onDone, hiddenActions = [], presetValues, schemaOverride }) {
  const searchParams = useSearchParams();
  const schema = schemaOverride || ContractorsSchema;
  const id = idProp ?? searchParams.get(`${ContractorsSchema.entity}_dataNode`);
  const form = useEntityFormController(schema, ContractorsActions, {
    id,
    onDone,
