...
// AttendanceProfile — pure shell. It resolves the id, wires up the
...
export default function AttendanceProfile({ id: idProp, onDone, hiddenActions = [], presetValues, schemaOverride }) {
  const searchParams = useSearchParams();
  const schema = schemaOverride || AttendanceSchema;
  const id = idProp ?? searchParams.get(`${AttendanceSchema.entity}_dataNode`);
  const form = useEntityFormController(schema, AttendanceActions, {
    id,
    onDone,
