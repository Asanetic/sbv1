...
// WorkscheduleProfile — pure shell. It resolves the id, wires up the
...
export default function WorkscheduleProfile({ id: idProp, onDone, hiddenActions = [], presetValues, schemaOverride }) {
  const searchParams = useSearchParams();
  const schema = schemaOverride || WorkscheduleSchema;
  const id = idProp ?? searchParams.get(`${WorkscheduleSchema.entity}_dataNode`);
  const form = useEntityFormController(schema, WorkscheduleActions, {
    id,
    onDone,
